...
class BaseProcessor(object):
    ...
    def preprocess(self, blob):
        with tf.variable_scope(_('preprocess')):
            ...
            shape = blob.labels.get_shape().as_list()
            new_shape = [shape[0]] + [self.num_repeats] + shape[1:]
            expand = [1]*len(new_shape)
            expand[1] = self.num_repeats
	    new_shape = [-1] + new_shape[2:]
            self.labels = tf.reshape(tf.tile(tf.expand_dims(blob.labels, 1), expand), new_shape)
